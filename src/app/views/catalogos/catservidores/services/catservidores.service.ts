...
export class CatservidoresService {
  private modals: any[] = [];
  ...
  async getAdmin(dataTablesParameters: any): Promise<any> {
    ...
    try {
      ...
      let req:any = dataTablesParameters,
          datos:any = "",
          query = "";
      //const rawData = DB().query(`SELECT * FROM catservidores where id=?`, [1]);
  ...
  async getRecord(id: any): Promise<any> {
    ...
    this.conn= await this.dbSvc.connection;
    const rep = await this.conn.manager.getRepository(Catservidores)
    const Catservidor=await rep.findOne({    id: id })
    if (!Catservidor) {
        return { message: "Catservidor Not found." };
    }
    return Catservidor;
  }
  ...
  async setRecord(dataPack, actionForm): Promise<any>  {
    ...
    const dataVSchema = {
        ...
        departamento: { type: "string", empty: false },
        puesto: { type: "string", empty: false },
    };
    ...
    this.conn= await this.dbSvc.connection;
    const rep = await this.conn.manager.getRepository(Catservidores)
    const catservidores =  await rep.findOne({    id: dataPack.id })
