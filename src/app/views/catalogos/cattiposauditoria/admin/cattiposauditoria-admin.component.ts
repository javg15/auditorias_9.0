...
@Component({
  selector: 'app-cattiposauditoria-admin',
  templateUrl: './cattiposauditoria-admin.component.html',
})


export class CattiposauditoriaAdminComponent implements OnInit {
  ...
  nombreModulo = 'Cattiposauditoria';

  headersAdmin: [];

  cattiposauditoriaList: Cattiposauditoria[];

  constructor(private cattiposauditoriaService: CattiposauditoriaService, private route: ActivatedRoute,
    ) {     }
  ...
  ngOnInit(): void {
    ...
    this.dtOptions = {
      ...
      ajax: async (dataTablesParameters: any, callback) => {
        ...
        var self = this;
        await this.cattiposauditoriaService.getAdmin(dataTablesParameters).then(function(resp:any){
  ...
  openModal(id: string, accion: string, idItem: number) {
    this.cattiposauditoriaService.open(id, accion, idItem);
  }

  closeModal(id: string) {
    this.cattiposauditoriaService.close(id);
  }
