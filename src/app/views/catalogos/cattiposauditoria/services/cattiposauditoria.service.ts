...
export class CattiposauditoriaService {
  private modals: any[] = [];
  ...
  async getAdmin(dataTablesParameters: any): Promise<any> {
    ...
    try {
      ...
      let req:any = dataTablesParameters,
          datos:any = "",
          query = "";
      //const rawData = DB().query(`SELECT * FROM cattiposauditoria where id=?`, [1]);
  ...
  async getRecord(id: any): Promise<any> {
    ...
    this.conn= await this.dbSvc.connection;
    const rep = await this.conn.manager.getRepository(Cattiposauditoria)
    const Cattipoauditoria=await rep.findOne({    id: id })
    if (!Cattipoauditoria) {
        return { message: "Cattipoauditoria Not found." };
    }
    return Cattipoauditoria;
  }
  ...
  async setRecord(dataPack, actionForm): Promise<any>  {
    ...
    this.conn= await this.dbSvc.connection;
    const rep = await this.conn.manager.getRepository(Cattiposauditoria)
    const cattiposauditoria =  await rep.findOne({    id: dataPack.id })
    dataPack.state = gral.GetStatusSegunAccion(actionForm);

    if (!cattiposauditoria) {
        delete dataPack.id;
