...
import { Catestatus } from 'src/app/_data/_models/catestatus';
...
export class CatestatusService {
  ...
  async getAdmin(dataTablesParameters: any): Promise<any> {
    ...
    try {
      ...
      let req:any = dataTablesParameters,
          datos:any = "",
          query = "";
      //const rawData = DB().query(`SELECT * FROM catestatus where id=?`, [1]);
  ...
  async getRecord(id: any): Promise<any> {
    ...
    this.conn= await this.dbSvc.connection;
    const rep = await this.conn.manager.getRepository(Catestatus)
    const catestatus=await rep.findOne({    id: id })
    if (!catestatus) {
        return { message: "Catestatus Not found." };
    }
    return catestatus;
  }
  ...
  async setRecord(dataPack, actionForm): Promise<any>  {
    ...
    this.conn= await this.dbSvc.connection;
    const rep = await this.conn.manager.getRepository(Catestatus)
    const catestatus =  await rep.findOne({    id: dataPack.id })
