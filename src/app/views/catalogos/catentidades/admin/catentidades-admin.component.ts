...
@Component({
  selector: 'app-catentidades-admin',
  templateUrl: './catentidades-admin.component.html',
})


export class CatentidadesAdminComponent implements OnInit {
  ...
  nombreModulo = 'Catentidades';

  headersAdmin: [];

  catentidadesList: Catentidades[];

  constructor(private catentidadesService: CatentidadesService, private route: ActivatedRoute,
    ) {     }
  ...
  ngOnInit(): void {
    ...
    this.dtOptions = {
      ...
      ajax: async (dataTablesParameters: any, callback) => {
        ...
        var self = this;
        await this.catentidadesService.getAdmin(dataTablesParameters).then(function(resp:any){
  ...
  openModal(id: string, accion: string, idItem: number) {
    this.catentidadesService.open(id, accion, idItem);
  }

  closeModal(id: string) {
    this.catentidadesService.close(id);
  }
