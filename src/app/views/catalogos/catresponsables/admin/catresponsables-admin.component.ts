...
@Component({
  selector: 'app-catresponsables-admin',
  templateUrl: './catresponsables-admin.component.html',
})


export class CatresponsablesAdminComponent implements OnInit {
  ...
  nombreModulo = 'Catresponsables';

  headersAdmin: [];

  catresponsablesList: Catresponsables[];

  constructor(private catresponsablesService: CatresponsablesService, private route: ActivatedRoute,
    ) {     }
  ...
  ngOnInit(): void {
    ...
    this.dtOptions = {
      ...
      ajax: async (dataTablesParameters: any, callback) => {
        ...
        var self = this;
        await this.catresponsablesService.getAdmin(dataTablesParameters).then(function(resp:any){
  ...
  openModal(id: string, accion: string, idItem: number) {
    this.catresponsablesService.open(id, accion, idItem);
  }

  closeModal(id: string) {
    this.catresponsablesService.close(id);
  }
