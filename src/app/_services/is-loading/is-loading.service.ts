...
export class IsLoadingService {
  ...
  isLoading(args: IGetLoadingOptions = {}): boolean {
    ...
    return keys
      .map((key)=>{
        let dato=(this.loadingSubjects.get(key)===null || this.loadingSubjects.get(key)===undefined?undefined:this.loadingSubjects.get(key).value) 
  ...
  add(
    ...
    const keys = this.normalizeKeys(options===null || options===undefined ? undefined : options.key);
    ...
    keys.forEach((key, keyIndex) => {
      ...
      if (options===null || options===undefined ? undefined : options.unique) {
        const index = loadingStack.findIndex((t) =>
          t.isSame(sub, options===null || options===undefined ? undefined : options.unique)
        );
      ...
      loadingStack.push(
        new LoadingToken(
          ...
          options===null || options===undefined ? undefined : options.unique
        )
      );
